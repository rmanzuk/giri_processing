% Script and functions to downsample a grinder image stack both pixel-wise
% and in the z-direction
% Ryan A. Manzuk
% Created 05/28/2020
% Last edited 05/28/2020

in = '/Users/rmanzuk/Desktop/nevada_jan_2019/sm_117_71_reconstruction/cleaned_binary_tifs';
out = '/Users/rmanzuk/Desktop/nevada_jan_2019/sm_117_71_reconstruction/downsampled_stack';

downsample_im_stack(in,'*.tiff',out,1,0.5,4)


%% Funcion to downsample an image stack by desired pixel resolution and z resolution
function [] = downsample_im_stack(input_directory,input_ext,output_directory,tif_to_jpeg,pixel_scaling,z_sample)
%
% IN 
% input_directory: string directory with source folder with the full-resolution image stack
%
% input_ext: string file extention for the input images. example '*.tiff'
%
% output_directory: string directory where you want to put the downsampled
% stack
%
% tif_to_jpeg: binary logical flag if input files are .tif and you want to
% output as .jpeg to further reduce file size
%
% pixel_scaling: fraction (0.01 to 1.0) by which to scale pixel resolution
% to downsample each individual image
%
% z_sample: interval by which to sample images in the z direction. So if you want to
% read every 4th image, z_sample would be 4.
%
% OUT

% R. A. Manzuk, 05/28/2020

    %% begin the function
    
    % set up file reading of the input directory in correct order
    file_pattern = fullfile(input_directory, input_ext);
    input_files = dir(file_pattern);
    ordered_files = natsortfiles({input_files.name});
    
    % read each desired image, downsample, and save in the proper folder
    for k = 1:floor(numel(ordered_files)/z_sample)
        file_to_read = fullfile(input_directory, ordered_files{k*z_sample});
        file_to_write = fullfile(output_directory, ordered_files{k*z_sample});
        [~,image_name,~] = fileparts(file_to_read);
        fprintf('Now reading %s\n', file_to_read);
        image = imread(file_to_read);
        downsampled_image = imresize(image,pixel_scaling);
        if tif_to_jpeg
            imwrite(uint8(downsampled_image),file_to_write,'jpg')
        else
            imwrite(downsampled_image,file_to_write,'tiff')
        end
    end
end

%% natsort function to ensure going through image stack in proper order
